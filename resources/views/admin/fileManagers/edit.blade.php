...
<div class="card">
    ...
    <div class="card-body">
        <form method="POST" action="{{ route("admin.file-managers.update", [$fileManager->id]) }}" enctype="multipart/form-data">
            ...
            <div class="form-group">
                <label for="indentures">{{ trans('cruds.fileManager.fields.indenture') }}</label>
                ...
                <select class="form-control select2 {{ $errors->has('indentures') ? 'is-invalid' : '' }}" name="indentures[]" id="indentures" multiple>
                    @foreach($indentures as $id => $indenture)
                        <option value="{{ $id }}" {{ (in_array($id, old('indentures', [])) || $fileManager->indentures->contains($id)) ? 'selected' : '' }}>{{ $indenture }}</option>
                    @endforeach
                </select>
                ...
                @endif
                <span class="help-block">{{ trans('cruds.fileManager.fields.indenture_helper') }}</span>
            </div>
            <div class="form-group">
